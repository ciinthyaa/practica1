...
**Respuesta:** ¿Que necesitamos saber/conocer?¿Cuál es el usuario
culpable?¿Cómo lo hizo?¿Qué datos tenemos? - Los registros de conexiones
TCP de cada máquina de cada trabajador. - Conocemos que clientes se
conectan desde lugares remotos de forma legítima. - Sabemos cuáles son
los servicios legítimos que tenemos activos. - Los puertos que se
mantienen como lícitos para el uso de los servicios.
...
Conclusiones y toma de decisiones según los resultados: - Analizar si el
uso de esos servicios es realmente requerido para los usuarios. -
Concientizar sobre el uso adecuado de las políticas establecidas en la
organización. - Implementar políticas que bloqueen el uso tanto de
puertos como de servicios ilegítimos.
...
Para responder el literal 1 y 2, haremos uso de funciones, para el
literal 1 la función dim y para el literal 2 la funcion mean.
...
1.  ¿Cuáles son las dimensiones del dataset cargado (número de filas y
    ...
    > ```
    >
    > *Respuesta:* Existen 47748 columnas y 7 filas
...
2.  Valor medio de la columna Bytes
    ...
    ``` r
    epa_http2$Bytes[ is.na (epa_http2$Bytes)] <- 0
    ```

    Después hacemos uso de la función y ya podemos obtener el valor
    medio.
    ...
    *Respuesta:* El valor medio es de 6531.457
...
-   Separamos variable(columnas) en varias columnas epa_http \<-
    epa_http %\>% separate(Timestamp,paste0("Timestamp",1:4),sep=':')

-   Unimos variables (columnas) epa_http \<- epa_http %\>%
    unite(Timestamp2,Timestamp3,Timestamp4, col = Time, sep = ":",
    remove = FALSE)%\>%

-   Quitamos el parametro "]" epa_http$Time<-substring(epa_http$Time, 1,

    8)  epa_http$Timestamp1<-substring(epa_http$Timestamp1, 2, 3)

-   Selección de tipo de petición
    epa_http$Phoras <- ifelse (str_equal("GET",epa_http$Tipo) ==TRUE,
    epa_http\$Timestamp2, NA )

-   Conteo de horas y creación de nueva tabla copy_epa \<-epa_http %\>%
    group_by(Phoras) %\>% tally()

-   Hora con más peticiones copy_epa[15, 1] max(copy_epa\$n)

-   Hora con más peticiones, segunda opción Qhora \<-
    copy_epa$Phoras[which.max(copy_epa$n)] print(Qhora)
...
epa_http$compa <- ifelse( (str_detect(epa_http$IPs, ".edu"))==TRUE &
(str_detect(epa_http$URL, ".txt")) == TRUE, epa_http$Bytes, NA )
epa_http$compa[is.na(epa_http$compa)] \<- 0 sum(epa_http\$compa)
